...
REST is *not* collection/item CRUD; frameworks which fail to realise this (e.g. Zend Framework, Ruby on Rails) are introducing needless restriction - the only (debateable) 'benefit' of which is that your collection and item resources can be handled in one controller and with one route.
